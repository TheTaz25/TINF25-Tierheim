import { removeClass, addClass, setInnerText, setValue } from "./dom";
...
type Translation = Record<StatName, Record<"low" | "medium" | "high", string>>;
...
function connectDebugScreen() {
  ...
  if (input) {
    input.addEventListener("change", (event) => {
      ...
      const targetScreen = parseInt(target.value);
...
function connectDebugToStat(statName: string) {
